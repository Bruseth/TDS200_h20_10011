...
const Profile = (props: any) => {
  ...
  console.log(data?.posts[0].description)
  ...
  return (
    <IonPage>
      ...
      <IonContentStyled fullscreen>
        <IonCard>
          <IonCardHeader> {data?.posts.map((post) => { 
            return<IonCardTitle key={post.id}>{post.user.display_name}</IonCardTitle>
                ...
                <IonList>
                  {
                    data?.posts.map((post) => {
                      ...
                      return <IonItem key={post.id}> {post.title}</IonItem>
